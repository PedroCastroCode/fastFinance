...
export default class UsersInMemoryRepository
  extends InMemoryRepository<Users>
  ...
  async IsUsernameAlreadyInUse(
    ...
    return (
      this.items.findIndex(
        (x) => x.username === username && x.id.toString() !== id,
      ) >= 0
    );
  }

  async GetUserByEmail(email: string): Promise<Users | null | undefined> {
    return this.items.find((x) => x.email === email);
  }
}
